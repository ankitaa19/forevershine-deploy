...
@section('content')
<section class="content modalboxs">
						...
						<form id="form" action="{{ url('admin/catalog/variant/add') }}" method="post" enctype="multipart/form-data" autocomplete="off">
							...
							<div class="row clearfix" style="text-align: right;">
							  <div class="change m-0">
								...
								<button style="background:#353c48;padding:0 35px;" type="button" class="btn btn-primary waves-effect m-r-15 add-more" >Add New</button> 
							  </div>
							</div>
							<div id="mySortable" >
								@if(!empty($variantResult))
									@foreach($variantResult as $key=>$attr)
										<div class="row clearfix after-add-more row ">
											...
											<input type="hidden" name="sort_order[]" value="{{ $attr['sort_order'] }}" class="sort_order">
